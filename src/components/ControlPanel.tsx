...
export const ControlPanel: React.FC<Props> = ({
  onImageUpload, prompt, onPromptChange, onGenerate, isLoading, uploadedImage
}) => {
    ...
    <section className="bg-white/5 rounded-xl p-4 sm:p-6 border border-white/10">
      <div className="space-y-4">
        <div>
          ...
          <input type="file" accept="image/*" onChange={handleFile} className="block w-full text-sm text-white/80"/>
          {(localPreview || uploadedImage) && (
            <img src={localPreview || uploadedImage || undefined} alt="preview"
        ...
        <div>
          ...
          <textarea value={prompt} onChange={(e) => onPromptChange(e.target.value)}
            placeholder="Ví dụ: Làm sáng ảnh, tăng chi tiết khuôn mặt, đổi nền mờ…"
